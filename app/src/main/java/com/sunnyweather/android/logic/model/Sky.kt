...
/*编写一个额外的转换函数。因为彩云天气返回的数据中，天气情况都是一些诸如CLOUDY、WIND之类的天气代码，需要编写一个转换函数将这些天气代码转换成一个Sky对象
* 首先定义Sky类作为数据模型，它包含了info、icon和bg这3个字段，分别表示该天气情况所对应的文字、图标和背景。
* 然后使用mapOf()函数来定义每种天气代码所应该对应的文字、图标和背景。不过没能给每种天气代码都准备一份对应的图标与背景，
因此对于一些类型比较相近的天气，这里就使用同一份图标或背景了。
* 最后，定义getSky()方法来根据天气代码获取对应的Sky对象
* */
